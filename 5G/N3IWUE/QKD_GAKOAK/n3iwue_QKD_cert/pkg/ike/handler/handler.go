...
const (
	Alice2_IP     = "10.98.1.139:9120"
	Bob_IP        = "158.227.38.81:9120"
	Alice2_SAE_ID = "alice2_sae_etsi_1"
	Bob_SAE_ID    = "bob_sae_etsi_1"
)
...
func HandleIKESAINIT(
	...
	// jsonData := notifications[0].NotificationData

	// resp, err := http.Post("http://"+Bob_IP+"/api/v1/keys/"+Alice2_SAE_ID+"/dec_keys",
	// 	"application/json",
	// 	bytes.NewBuffer(jsonData))
	// if err != nil {
	// 	log.Fatalf("Error making request to Bob: %v", err)
	// }
	// defer resp.Body.Close()

	// body, err := io.ReadAll(resp.Body)
	// if err != nil {
	// 	log.Fatalf("Error reading response from Bob: %v", err)
	// }

	// var responseData KeysContainer
	// err = json.Unmarshal(body, &responseData)
	// if err != nil {
	// 	log.Fatalf("Error unmarshaling response from Bob: %v", err)
	// }

	// fmt.Println("RESPONSE FROM BOB:")
	// fmt.Printf("%+v\n", responseData)

	// if len(responseData.Keys) < 4 {
	// 	log.Fatalf("Expected at least 4 keys, got %d", len(responseData.Keys))
	// }

	// ItoRENC := responseData.Keys[0].Key
	// RtoIENC := responseData.Keys[1].Key
	// ItoRINT := responseData.Keys[2].Key
	// RtoIINT := responseData.Keys[3].Key
	...
	// 	RtoIINT,
	// }
	// keyBytesTemp := make([][]byte, 4)
	keyBytes := make([][]byte, 4)

	// for i, b64Str := range b64Strings {
	// 	var err error
	// 	keyBytesTemp[i], err = base64.StdEncoding.DecodeString(b64Str)
	// 	if err != nil {
	...
	// keyBytes[0] = keyBytesTemp[0]
	// keyBytes[1] = keyBytesTemp[1]
	// keyBytes[2] = keyBytesTemp[2][:20]
	// keyBytes[3] = keyBytesTemp[3][:20]

	ItoRENC := "7a4f3b2c1d0e9f8a7b6c5d4e3f2a1b0c9d8e7f6a5b4c3d2e1f0a9b8c7d6e5f4a"
	RtoIENC := "3e5f8a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f"
	ItoRINT := "3e8a9f7b2c4d1e0a5b6c7d8e9f0a1b2c3d4e5f6a"
	RtoIINT := "a1b2c3d4e5f60718293a4b5c6d7e8f90a1b2c3d4"

	keyBytes[0], err = hex.DecodeString(ItoRENC)
	if err != nil {
		fmt.Printf("Error decoding b64 string: %v\n", err)
		return
	}
	keyBytes[1], err = hex.DecodeString(RtoIENC)
	if err != nil {
		fmt.Printf("Error decoding b64 string: %v\n", err)
		return
	}
	keyBytes[2], err = hex.DecodeString(ItoRINT)
	if err != nil {
		fmt.Printf("Error decoding b64 string: %v\n", err)
		return
	}
	keyBytes[3], err = hex.DecodeString(RtoIINT)
	if err != nil {
		fmt.Printf("Error decoding b64 string: %v\n", err)
		return
	}

	responseIKEMessageData, err := message.Encode()
	if err != nil {
	...
	ikeSecurityAssociation := &context.IKESecurityAssociation{
		...
		InitiatorSignedOctets: append(n3ueSelf.N3IWFUe.N3IWFIKESecurityAssociation.InitiatorSignedOctets, remoteNonce...),
		ResponderSignedOctets: append(responseIKEMessageData, n3ueSelf.LocalNonce...),
		UEIsBehindNAT:         ueIsBehindNAT,
		N3IWFIsBehindNAT:      n3iwfIsBehindNAT,
	}
	ikeSecurityAssociation.IKESAKey.Integ_i = ikeSecurityAssociation.IKESAKey.IntegInfo.Init(ikeSecurityAssociation.IKESAKey.SK_ai)
	ikeSecurityAssociation.IKESAKey.Integ_r = ikeSecurityAssociation.IKESAKey.IntegInfo.Init(ikeSecurityAssociation.IKESAKey.SK_ar)
	ikeSecurityAssociation.IKESAKey.Prf_i = ikeSecurityAssociation.IKESAKey.PrfInfo.Init(ikeSecurityAssociation.IKESAKey.SK_pi)
	ikeSecurityAssociation.IKESAKey.Prf_r = ikeSecurityAssociation.IKESAKey.PrfInfo.Init(ikeSecurityAssociation.IKESAKey.SK_pr)
	ikeSecurityAssociation.IKESAKey.Encr_i, err = ikeSecurityAssociation.IKESAKey.EncrInfo.NewCrypto(ikeSecurityAssociation.IKESAKey.SK_ei)
	if err != nil {
		ikeLog.Errorf("Generate key for IKE SA failed: %+v", err)
		return
	}
	ikeSecurityAssociation.IKESAKey.Encr_r, err = ikeSecurityAssociation.IKESAKey.EncrInfo.NewCrypto(ikeSecurityAssociation.IKESAKey.SK_er)
	if err != nil {
		ikeLog.Errorf("Generate key for IKE SA failed: %+v", err)
		return
	}
...
func HandleIKEAUTH(
	...
	var auth *ike_message.Authentication
	var rsaPublicKey *rsa.PublicKey
	var IDr *ike_message.IdentificationResponder
	var err error
	var ok bool
	...
	for _, ikePayload := range message.Payloads {
		...
		case ike_message.TypeIDr:
			ikeLog.Info("Get IDr")
			IDr = ikePayload.(*ike_message.IdentificationResponder)
		case ike_message.TypeAUTH:
			ikeLog.Info("Get AUTH")
			auth = ikePayload.(*ike_message.Authentication)
		case ike_message.TypeSA:
		...
		case ike_message.TypeN:
			...
			if notification.NotifyMessageType == ike_message.QKD_KEY {
				//jsonData = notification.NotificationData
			}
	...
	case IKEAUTH_Request:
		...
		if certificate != nil {
			parsedCert, err := x509.ParseCertificate(certificate.CertificateData)
			if err != nil {
			...
			// 	fmt.Println("\nCertificate Details:")
			// 	fmt.Println("Subject:", parsedCert.Subject)
			// 	fmt.Println("Issuer:", parsedCert.Issuer)
			// 	fmt.Println("NotBefore:", parsedCert.NotBefore)
			// 	fmt.Println("NotAfter:", parsedCert.NotAfter)
			// 	fmt.Println("Public Key:", rsaPublicKey)
		}

		//Verify AUTH payload with certificate
		var idPayload ike_message.IKEPayloadContainer
		idPayload.BuildIdentificationResponder(IDr.IDType, IDr.IDData)
		...
		//fmt.Println("Lenght:", len(ikeSecurityAssociation.ResponderSignedOctets))
		...
		//fmt.Printf("Raw Data (Verification):\n%s", hex.Dump(ikeSecurityAssociation.ResponderSignedOctets))
		sha1HashFunction := sha1.New() // #nosec G401
		_, err = sha1HashFunction.Write(ikeSecurityAssociation.ResponderSignedOctets)
		if err != nil {
			fmt.Println("Failed to hash authentication data:", err)
		}
		hashed := sha1HashFunction.Sum(nil)
		fmt.Printf("Hashed data (Verification): %x\n", hashed)

		err = rsa.VerifyPKCS1v15(rsaPublicKey, crypto.SHA1, hashed, auth.AuthenticationData)
		if err != nil {
			ikeLog.Errorf("HandleIKEAUTH() EAP_RegistrationRequest: %v", err)
			return
		}
	...
	case IKEAUTH_Authentication:
		...
		var childSecurityAssociationContext = new(context.ChildSecurityAssociation)

		childSecurityAssociationContext.OutboundSPI = outboundSPI
		childSecurityAssociationContext.InboundSPI = factory.N3ueInfo.IPSecSaCpSPI
		...
		childSAKey := new(ike_security.ChildSAKey)
		childSAKey.EncrKInfo = encr.StrToKType(encr.ENCR_AES_CBC_256)
		childSAKey.IntegKInfo = integ.StrToKType(integ.AUTH_HMAC_SHA1_96)
		// if jsonData != nil {
		// 	resp, err := http.Post("http://"+Bob_IP+"/api/v1/keys/"+Alice2_SAE_ID+"/dec_keys",
		// 		"application/json",
		// 		bytes.NewBuffer(jsonData))
		...
		childSAKey.InitiatorToResponderEncryptionKey = keyBytes[0]
		childSAKey.InitiatorToResponderIntegrityKey = keyBytes[2]
		childSAKey.ResponderToInitiatorEncryptionKey = keyBytes[1]
		childSAKey.ResponderToInitiatorIntegrityKey = keyBytes[3]

		childSecurityAssociationContext.ChildSAKey = childSAKey
		childSecurityAssociationContext.EsnInfo, err = esn.StrToType(esn.String_ESN_DISABLE)
		if err != nil {
			fmt.Printf("ESN string not defined: %v\n", err)
			return
		}
		// } else {
		// 	log.Fatalf("JSON data is nil\n")
		// 	return
		// }
...
func HandleCREATECHILDSA(
	...
	// resp, err := http.Post("http://"+Bob_IP+"/api/v1/keys/"+Alice2_SAE_ID+"/enc_keys",
	// 	"application/json",
	// 	bytes.NewBuffer(jsonData))
	// if err != nil {
	// 	log.Fatalf("Error making request to Alice: %v", err)
	// }
	// defer resp.Body.Close()
	...
	// fmt.Println("KEYS FROM BOB:")
	// fmt.Printf("%+v\n", keys)

	// // Check if we got enough keys
	// if len(keys.Keys) < 4 {
	// 	log.Fatalf("Expected at least 4 keys, got %d", len(keys.Keys))
	// }

	// ItoRENC := keys.Keys[0].Key
	// RtoIENC := keys.Keys[1].Key
	// ItoRINT := keys.Keys[2].Key
	// RtoIINT := keys.Keys[3].Key
	...
	// Prepare key IDs for second request
	// keyIDs := make([]KeyID, 4)
	// for i := 0; i < 4; i++ {
	// 	keyIDs[i].KeyID = keys.Keys[i].KeyID
	// }

	// requestBodyIDs := KeyIDsRequest{
	// 	KeyIDs: keyIDs,
	// }

	// jsonData, err = json.Marshal(requestBodyIDs)
	// if err != nil {
	// 	log.Fatalf("Error marshaling key IDs: %v", err)
	// }
	...
	//Gakoen id-ak bidali UEari notification payload batean
	// ikePayload.BuildNotification(ike_message.TypeNone, ike_message.QKD_KEY, nil, jsonData)
	...
	// 	-1,
	// )
	childSecurityAssociationContextUserPlane := new(context.ChildSecurityAssociation)
	childSecurityAssociationContextUserPlane.OutboundSPI = OutboundSPI
	childSecurityAssociationContextUserPlane.InboundSPI = binary.BigEndian.Uint32(inboundSPI)
	...
	childSAKey := new(ike_security.ChildSAKey)
	//Gakoak irakurri eta sartu hemen
	childSAKey.EncrKInfo = encr.StrToKType(encr.ENCR_AES_CBC_256)
	childSAKey.IntegKInfo = integ.StrToKType(integ.AUTH_HMAC_SHA1_96)
	...
	childSecurityAssociationContextUserPlane.ChildSAKey = childSAKey
	childSecurityAssociationContextUserPlane.EsnInfo, err = esn.StrToType(esn.String_ESN_DISABLE)
	if err != nil {
		fmt.Printf("ESN string not defined: %v\n", err)
		return
	}
