...
func SendIKEAUTH() {
	...
	//For InitiatorSignedOctets
	idPayloadData, err := ikePayload.Encode()
	if err != nil {
	...
	ikeSA.InitiatorSignedOctets = append(
		ikeSA.InitiatorSignedOctets, ikeSA.Prf_i.Sum(nil)...)

	//Ziurtagiria lortu eta bidali
	certFile, err := os.ReadFile("/home/gaizka/GrAL_EVO/SSL/ca-cert.pem")
	if err != nil {
		ikeLog.Error("Error reading cert file:", err)
		return
	}

	UECertificate, _ := pem.Decode(certFile)
	if UECertificate == nil {
		ikeLog.Error("No PEM data found in certificate file")
		return
	}
	if UECertificate.Bytes != nil {
		ikePayload.BuildCertificate(ike_message.X509CertificateSignature, UECertificate.Bytes)
	}

	//Gako pribatua lortu
	keyFile, err := os.ReadFile("/home/gaizka/GrAL_EVO/SSL/ca-key-rsa.pem")
	if err != nil {
		ikeLog.Error("Error reading private key file:", err)
		return
	}

	keyData, _ := pem.Decode(keyFile)
	if keyData == nil {
		ikeLog.Error("No PEM data found in key file")
		return
	}

	privateKey, err := x509.ParsePKCS1PrivateKey(keyData.Bytes)
	if err != nil {
		ikeLog.Error("Error parsing private key:", err)
		return
	}

	//Sinadura egin eta bidali
	sha1HashFunction := sha1.New() // #nosec G401
	_, err = sha1HashFunction.Write(ikeSA.InitiatorSignedOctets)
	if err != nil {
		ikeLog.Errorf("Hash function write error: %v", err)
		return
	}

	signature, err := rsa.SignPKCS1v15(rand.Reader, privateKey, crypto.SHA1, sha1HashFunction.Sum(nil))
	if err != nil {
		ikeLog.Fatal("Error signing message:", err)
		return
	}

	ikePayload.BuildAuthentication(ike_message.RSADigitalSignature, signature)
...
func SendIKEMessageToN3IWF(
	...
	//Paketea zifratu gabe bidaltzeko??
