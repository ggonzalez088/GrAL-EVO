...
func (s *Server) HandleIKESAINIT(
	...
	var IKESAkey = new(ike_security.IKESAKey)
	IKESAkey.EncrInfo = encr.DecodeTransform(chooseProposal[0].EncryptionAlgorithm[0])
	IKESAkey.IntegInfo = integ.DecodeTransform(chooseProposal[0].IntegrityAlgorithm[0])
	IKESAkey.PrfInfo = prf.DecodeTransform(chooseProposal[0].PseudorandomFunction[0])
	...
	// resp, err := http.Post("http://"+Alice2_IP+"/api/v1/keys/"+Bob_SAE_ID+"/enc_keys",
	// 	"application/json",
	...
	// if err != nil {
	// 	log.Fatalf("Error reading response from Alice: %v", err)
	// }

	// var keys KeysContainer
	// err = json.Unmarshal(body, &keys)
	// if err != nil {
	// 	log.Fatalf("Error unmarshaling response from Alice: %v", err)
	// }

	// fmt.Println("KEYS FROM ALICE:")
	// fmt.Printf("%+v\n", keys)
	...
	IKESAkey.SK_ei = keyBytes[0]
	IKESAkey.SK_er = keyBytes[1]
	IKESAkey.SK_ai = keyBytes[2]
	IKESAkey.SK_ar = keyBytes[3]
	IKESAkey.SK_pi = keyBytes[2]
	IKESAkey.SK_pr = keyBytes[3]
	...
	ikeSecurityAssociation.IKESAKey = IKESAkey
	...
	//responseIKEPayload.BuildNotification(ike_message.TypeNone, ike_message.QKD_KEY, nil, jsonData)
...
func (s *Server) HandleIKEAUTH(
	...
	case PreSignalling:
		...
		//Ziurtagiria baieztatu
		if certificate != nil && authentication != nil {
			...
			_, err = sha1HashFunction.Write(ikeSecurityAssociation.InitiatorSignedOctets)
			if err != nil {
				ikeLog.Error("Failed to hash authentication data:", err)
			}
			hashed := sha1HashFunction.Sum(nil)
			ikeLog.Tracef("Hashed data (Verification): %x\n", hashed)

			err = rsa.VerifyPKCS1v15(rsaPublicKey, crypto.SHA1, hashed, authentication.AuthenticationData)
			if err != nil {
...
func (s *Server) HandleCREATECHILDSA(
	...
	// fmt.Printf("%+v\n", jsonData)
	// if jsonData != nil {
	// 	resp, err := http.Post("http://"+Alice2_IP+"/api/v1/keys/"+Bob_SAE_ID+"/dec_keys",
	// 		"application/json",
	// 		bytes.NewBuffer(jsonData))
	// 	if err != nil {
	...
	ikeSecurityAssociation.SK_ei = keyBytes[0]
	ikeSecurityAssociation.SK_er = keyBytes[1]
	ikeSecurityAssociation.SK_ai = keyBytes[2]
	ikeSecurityAssociation.SK_ar = keyBytes[3]
...
func (s *Server) continueCreateChildSA(
	...
	childSAKey.InitiatorToResponderEncryptionKey = ikeSecurityAssociation.SK_ei
	childSAKey.ResponderToInitiatorEncryptionKey = ikeSecurityAssociation.SK_er
	childSAKey.InitiatorToResponderIntegrityKey = ikeSecurityAssociation.SK_ai
	childSAKey.ResponderToInitiatorIntegrityKey = ikeSecurityAssociation.SK_ar
...
func (s *Server) CreatePDUSessionChildSA(
	...
	for {
		if len(temporaryPDUSessionSetupData.UnactivatedPDUSession) > temporaryPDUSessionSetupData.Index {
			...
			encrTranform, err := encr.ToTransform(encr.StrToType(encr.ENCR_AES_CBC_256))
			if err != nil {
			...
			if pduSession.SecurityIntegrity {
				proposal.IntegrityAlgorithm = append(proposal.IntegrityAlgorithm,
					integ.ToTransform(integ.StrToType(integ.AUTH_HMAC_SHA2_256_128)))
			}
...
func SelectProposal(proposals ike_message.ProposalContainer) ike_message.ProposalContainer {
	...
	for _, proposal := range proposals {
		...
		var encryptionAlgorithmTransform *ike_message.Transform
		var pseudorandomFunctionTransform *ike_message.Transform
		var integrityAlgorithmTransform *ike_message.Transform
		// var diffieHellmanGroupTransform *ike_message.Transform
