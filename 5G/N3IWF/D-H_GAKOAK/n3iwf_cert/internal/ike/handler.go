...
func (s *Server) HandleIKESAINIT(
	...
	fmt.Println("Length:", hex.Dump(nonce.NonceData))
	...
	fmt.Println("Length:", len(ikeSecurityAssociation.ResponderSignedOctets))
...
func (s *Server) HandleIKEAUTH(
	...
	case PreSignalling:
		...
		ikeLog.Tracef("Hashed data (Signing): %x", sha1HashFunction.Sum(nil))
		var signedAuth []byte
